...
function sc_charge_card() {
	...
	if( has_action( 'sc_do_charge' ) ) {
		...
		try {
			...
			$query_args = array( 'error_code' => $e['error']['type'], 'charge_failed' => true );
		}
		...
		if ( isset( $_GET['charge'] ) && !isset( $_GET['charge_failed'] ) ) {
			...
			if ( empty( $sc_options['disable_success_message'] ) ) {
				...
				if ( isset( $_GET['store_name'] ) && ! empty( $_GET['store_name'] ) ) {
					$html .= __( 'From: ', 'sc' ) . esc_html( $_GET['store_name'] ) . '<br/>' . "\n";
				}
...
function sc_default_error_html( $html ) {
	...
	// PRO ONLY: Additional error code.
	$html .= '<p>' . sprintf( __( 'The returned error code was: %s', 'sc' ), esc_html( $_GET['error_code'] ) ) . '</p>' . "\n";

	$html .= '</div>' . "\n";

	return $html;
}
add_filter( 'sc_payment_details_error', 'sc_default_error_html' );
