...
class PrestaShopIntegrationModule_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $instance['only_if_products'] ) {
			global $post;
			$product_id = preg_replace( array('/^[\s,]+/', '/[\s,]+/', '/[\s,]+$/'), array('', ',', ''), get_post_meta( $post->ID, '_prestashop_integration_products', true ) );
			if ( !$product_id )
				return;
		}
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['only_if_products'] = $new_instance['only_if_products'] ? 1 : 0;
	...
	function form( $instance ) {
		...
		<p>
		<label for="<?php echo $this->get_field_id('only_if_products'); ?>"><?php _e('Only if products:'); ?></label> 
		<input type="checkbox" id="<?php echo $this->get_field_id('only_if_products'); ?>" name="<?php echo $this->get_field_name('only_if_products'); ?>" value="true" <?php echo $only_if_products ? 'checked' : ''; ?>/>
		</p>
		<?php 
	}
}
