	...
	<!--//--><![CDATA[//><!--
	    ...
	    function relative_time(time_value) {
	      ...
	      if (delta < 60) {
	        ...
	        return '<?php esc_attr_e( 'less than a minute ago', 'fascat' ); ?>';

	      } else if(delta < 120) {

	        return '<?php esc_attr_e( 'about a minute ago', 'fascat' ); ?>';

	      } else if(delta < (60*60)) {

	        return (parseInt(delta / 60)).toString() + ' <?php esc_attr_e( 'minutes ago', 'fascat' ); ?>';
	        ...
	        return 'about ' + (parseInt(delta / 3600)).toString() + ' <?php esc_attr_e( 'hours ago', 'fascat' ); ?>';
	        ...
	        return (parseInt(delta / 86400)).toString() + ' <?php esc_attr_e( 'days ago', 'fascat' ); ?>';
...
function woo_active_template( $filename = null ) {
	...
	trigger_error( sprintf( __( '%1$s is <strong>deprecated</strong> since version %2$s of the WooFramework! Please upgrade your Theme to the latest version.', 'fascat' ), 'woo_active_template', '5.4' ) );
...
function fascat_framework_update_page() {
	...
	delete_transient( 'wooframework_version_data' );
        ...
        $url = admin_url( 'admin.php?page=fascat_framework_update' );
...
function fascat_framework_update_head() {
  ...
  if( isset( $_REQUEST['page'] ) ) {
	...
	if( $_page == 'fascat_framework_update' ) {
		...
		if( $filesystem == false && $_POST['upgrade'] != 'Proceed' ) {
			...
			function fascat_framework_update_filesystem_warning() {
				...
				add_action( 'admin_notices', 'fascat_framework_update_filesystem_warning' );
		...
		$temp_file_addr = download_url( esc_url( 'http://www.fascat.com/updates/framework.zip' ) );
		...
		if ( is_wp_error($temp_file_addr) ) {
			...
			//The source file was not found or is invalid
				...
				function fascat_framework_update_missing_source_warning() {

					echo "<div id='source-warning' class='updated fade'><p>Failed: Invalid URL Provided</p></div>";

				}

				add_action( 'admin_notices', 'fascat_framework_update_missing_source_warning' );

			} else {

				function fascat_framework_update_other_upload_warning() {

					echo "<div id='source-warning' class='updated fade'><p>Failed: Upload - $error</p></div>";

				}

				add_action( 'admin_notices', 'fascat_framework_update_other_upload_warning' );
		...
		if ( is_wp_error($dounzip) ) {
			...
			if($error == 'incompatible_archive') {
				...
				function fascat_framework_update_no_archive_warning() {

					echo "<div id='woo-no-archive-warning' class='updated fade'><p>Failed: Incompatible archive</p></div>";

				}

				add_action( 'admin_notices', 'fascat_framework_update_no_archive_warning' );

			}

			if($error == 'empty_archive') {

				function fascat_framework_update_empty_archive_warning() {

					echo "<div id='woo-empty-archive-warning' class='updated fade'><p>Failed: Empty Archive</p></div>";

				}

				add_action( 'admin_notices', 'fascat_framework_update_empty_archive_warning' );

			}

			if($error == 'mkdir_failed') {

				function fascat_framework_update_mkdir_warning() {

					echo "<div id='woo-mkdir-warning' class='updated fade'><p>Failed: mkdir Failure</p></div>";

				}

				add_action( 'admin_notices', 'fascat_framework_update_mkdir_warning' );

			}

			if($error == 'copy_failed') {

				function fascat_framework_update_copy_fail_warning() {

					echo "<div id='woo-copy-fail-warning' class='updated fade'><p>Failed: Copy Failed</p></div>";

				}

				add_action( 'admin_notices', 'fascat_framework_update_copy_fail_warning' );
		...
		function fascat_framework_updated_success() {
		...
		add_action( 'admin_notices', 'fascat_framework_updated_success' );
...
add_action( 'admin_head', 'fascat_framework_update_head' );
...
function woo_get_fw_version( $url = '', $check_if_critical = false ) {
	...
	if( ! empty( $url ) ) {
    	...
    	$fw_url = 'http://www.fascat.com/updates/functions-changelog.txt';
...
if ( ! function_exists( 'woo_pagination' ) ) {
	...
	function woo_pagination( $args = array(), $query = '' ) {
		...
		$defaults = array(
			...
			'prev_text' => __( '&larr; Previous', 'fascat' ), // Translate in WordPress. This is the default.

			'next_text' => __( 'Next &rarr;', 'fascat' ), // Translate in WordPress. This is the default.
...
function woo_breadcrumbs( $args = array() ) {
	...
	$defaults = array(
		...
		'before' => '<span class="breadcrumb-title">' . __( 'You are here:', 'fascat' ) . '</span>',

		'after' => false,

		'front_page' => true,

		'show_home' => __( 'Home', 'fascat' ),
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				...
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'fascat' ) );



			elseif ( get_query_var( 'minute' ) )

				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'fascat' ), get_the_time( __( 'i', 'fascat' ) ) );



			elseif ( get_query_var( 'hour' ) )

				$trail['trail_end'] = get_the_time( __( 'g a', 'fascat' ) );
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				...
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'fascat' ) ) . '">' . get_the_time( __( 'Y', 'fascat' ) ) . '</a>';

				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'fascat' ) ) . '">' . get_the_time( __( 'F', 'fascat' ) ) . '</a>';

				$trail['trail_end'] = get_the_time( __( 'j', 'fascat' ) );
			...
			elseif ( get_query_var( 'w' ) ) {
				...
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'fascat' ), get_the_time( esc_attr__( 'W', 'fascat' ) ) );
			...
			elseif ( is_month() ) {
				...
				$trail['trail_end'] = get_the_time( __( 'F', 'fascat' ) );
			...
			elseif ( is_year() ) {
				...
				$trail['trail_end'] = get_the_time( __( 'Y', 'fascat' ) );
	...
	elseif ( is_search() )
		...
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'fascat' ), esc_attr( get_search_query() ) );
	...
	elseif ( is_404() )
		...
		$trail['trail_end'] = __( '404 Not Found', 'fascat' );
...
function woo_admin_bar_menu () {
	...
	// Main fascat Menu Item

	$wp_admin_bar->add_menu( array( 'id' => 'fascat', 'title' => $menu_label, 'href' => admin_url('admin.php?page=fascat') ) );



	// Theme Options

	$wp_admin_bar->add_menu( array( 'parent' => 'fascat', 'id' => 'fascat-theme-options', 'title' => __( 'Theme Options', 'fascat' ), 'href' => admin_url( 'admin.php?page=fascat' ) ) );



	if ( ( $super_user == $current_user_id ) || empty( $super_user ) ) {

		$wp_admin_bar->add_group( array( 'parent' => 'fascat', 'id' => 'fascat-super-user', 'meta' => array( 'class' => 'ab-sub-secondary' ) ) );



		// Framework Settings

		$wp_admin_bar->add_menu( array( 'parent' => 'fascat-super-user', 'id' => 'fascat-framework-settings', 'title' => __( 'Framework Settings', 'fascat' ), 'href' => admin_url( 'admin.php?page=fascat_framework_settings' ) ) );



		// Update Framework

		$wp_admin_bar->add_menu( array( 'parent' => 'fascat-super-user', 'id' => 'fascat-update-framework', 'title' => __( 'Update Framework', 'fascat' ), 'href' => admin_url( 'admin.php?page=fascat_framework_update' ) ) );
		...
		if ( true == $theme_data['is_child'] ) {
			...
			$child_theme_name = sprintf( __( 'Child Theme: %s %s', 'fascat' ), $theme_data['child_theme_name'], $theme_data['child_theme_version'] );

			$wp_admin_bar->add_menu( array( 'parent' => 'fascat-super-user', 'id' => 'fascat-child-theme-version-data', 'title' => $child_theme_name ) );

		}



		$theme_name = sprintf( __( 'Theme: %s %s', 'fascat' ), $theme_data['theme_name'], $theme_data['theme_version'] );
		$wp_admin_bar->add_menu( array( 'parent' => 'fascat-super-user', 'id' => 'fascat-theme-version-data', 'title' => $theme_name ) );



		$framework_version = sprintf( __( 'WooFramework: %s', 'fascat' ), $theme_data['framework_version'] );

		$wp_admin_bar->add_menu( array( 'parent' => 'fascat-super-user', 'id' => 'fascat-framework-version-data', 'title' => $framework_version ) );
...
function wooframework_add_woodojo_banner () {
	...
	$html .= '<span class="main">' . __( 'Enhance your theme with WooDojo.', 'fascat' ) . '</span>' . "\n";

	$html .= '<span>' . __( 'WooDojo is a powerful fascat features suite for enhancing your website. Learn more.', 'fascat' ) . '</span>' . "\n";

	$html .= '<a class="button-primary" href="' . esc_url( 'http://fascat.com/woodojo/' ) . '" title="' . esc_attr__( 'Get WooDojo', 'fascat' ) . '" target="_blank">' . __( 'Get WooDojo', 'fascat' ) . '</a>' . "\n";

	$html .= '<span class="close-banner"><a href="' . $close_url . '">' . __( 'Close', 'fascat' ) . '</a></span>' . "\n";
...
function wooframework_add_presstrends_banner () {
	...
	$html .= '<span class="main">' . __( 'Enable PressTrends', 'fascat' ) . '</span>' . "\n";

	$html .= '<span class="info">' . sprintf( __( 'PressTrends is a simple usage tracker that allows us to see how our customers are using fascat themes - so that we can help improve them for you. %sNone%s of your personal data is sent to PressTrends.', 'fascat' ), '<br /><strong>', '</strong>' ) . '</span>' . "\n";

	$html .= '<a class="button-primary" href="' . esc_url( admin_url( 'admin.php?page=fascat_framework_settings' ) ) . '" title="' . esc_attr__( 'Enable PressTrends', 'fascat' ) . '">' . __( 'Enable PressTrends', 'fascat' ) . '</a>' . "\n";
...
function wooframework_ajax_banner_close () {
	...
	if( ! current_user_can( 'install_plugins' ) ) wp_die( __( 'You do not have sufficient permissions to access this page.', 'fascat' ) );

	

	if( ! check_admin_referer( 'wooframework_banner_close' ) ) wp_die( __( 'You have taken too long. Please go back and retry.', 'fascat' ) );
 ...
 * Add a Timthumb Detection banner on all fascat Options screens.
...
function wooframework_add_wootimthumb_banner () {
	...
	if ( $thumb_php_test || $timthumb_php_test ) {
		...
		$html .= '<span class="main">' . __( 'ATTENTION: Insecure Version of Timthumb Image Resize Script Detected', 'fascat' ) . '</span>' . "\n";

		$html .= '<span>' . __( "A possible old version of the TimThumb script was detected in your theme folder. Please remove the following files from your theme as a security precaution", 'fascat' ) . ':</span>' . "\n";
 ...
 * Add a Static Front Page Detection banner on all fascat Options screens.
...
function wooframework_add_static_front_page_banner () {
	...
	$html .= '<span class="main">' . sprintf( __( 'You have setup a static front page in %1$sSettings > Reading%2$s.  Please set it to show "Your latest posts" if you want to display the default homepage in %3$s.', 'fascat' ), '<strong><a href="' . esc_url( admin_url( 'options-reading.php' ) ) . '">', '</a></strong>', $theme_data['theme_name'], '<strong>', '</strong>' ) . '</span>' . "\n";
...
function wooframework_display_theme_version_data ( $echo = true ) {
	...
	$html .= '<span class="framework">' . esc_html( sprintf( __( 'Framework %s', 'fascat' ), $data['framework_version'] ) ) . '</span>' . "\n";
