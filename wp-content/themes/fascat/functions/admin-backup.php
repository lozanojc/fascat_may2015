...
class fascat_Backup {
	...
	public function register_admin_screen () {
		...
		$this->admin_page = add_submenu_page('fascat', __( 'fascat Settings Backup', 'fascat' ), __( 'Backup Settings', 'fascat' ), 'manage_options', $this->token, array( &$this, 'admin_screen' ) );
...
?>
	<div class="wrap">
		...
		<h2><?php _e( 'Backup Settings', 'fascat' ); ?></h2>
		
		<h3><?php _e( 'Import Settings', 'fascat' ); ?></h3>
		
		<p><?php _e( 'If you have settings in a backup file on your computer, the WooFramework can import those into this site. To get started, upload your backup file to import from below.', 'fascat' ); ?></p>

		<div class="form-wrap">
			<form enctype="multipart/form-data" method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
				<?php wp_nonce_field( 'fascat-backup-import' ); ?>
				<label for="fascat-import-file"><?php printf( __( 'Upload File: (Maximum Size: %s)', 'fascat' ), ini_get( 'post_max_size' ) ); ?></label>
				<input type="file" id="fascat-import-file" name="fascat-import-file" size="25" />
				<input type="hidden" name="fascat-backup-import" value="1" />
				<input type="submit" class="button" value="<?php _e( 'Upload File and Import', 'fascat' ); ?>" />
			</form>
		</div><!--/.form-wrap-->
		
		<h3><?php _e( 'Export Settings', 'fascat' ); ?></h3>
		
		<p><?php _e( 'When you click the button below, the WooFramework will create a text file for you to save to your computer.', 'fascat' ); ?></p>
		<p><?php echo sprintf( __( 'This text file can be used to restore your settings here on "%s", or to easily setup another website with the same settings".', 'fascat' ), get_bloginfo( 'name' ) ); ?></p>
			
		<form method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
			<?php wp_nonce_field( 'fascat-backup-export' ); ?>
			<p><label><input type="radio" name="export-type" value="all"<?php checked( 'all', $export_type ); ?>> <?php _e( 'All Settings', 'fascat' ); ?></label>
<span class="description"><?php _e( 'This will contain all of the options listed below.', 'fascat' ); ?></span></p>

			<p><label for="content"><input type="radio" name="export-type" value="theme"<?php checked( 'theme', $export_type ); ?>> <?php _e( 'Theme Options', 'fascat' ); ?></label></p>
			<p><label for="content"><input type="radio" name="export-type" value="framework"<?php checked( 'framework', $export_type ); ?>> <?php _e( 'Framework Settings', 'fascat' ); ?></label></p>
			
			<input type="hidden" name="fascat-backup-export" value="1" />
			<input type="submit" class="button" value="<?php _e( 'Download Export File', 'fascat' ); ?>" />
		</form>
...
<?php
	...
	public function admin_screen_help ( $contextual_help, $screen_id, $screen ) {
		...
		$contextual_help =
		  '<h3>' . __( 'Welcome to the fascat Backup Manager.', 'fascat' ) . '</h3>' .
		  '<p>' . __( 'Here are a few notes on using this screen.', 'fascat' ) . '</p>' .
		  '<p>' . __( 'The backup manager allows you to backup or restore your "Theme Options" and other settings to or from a text file.', 'fascat' ) . '</p>' .
		  '<p>' . __( 'To create a backup, simply select the setting type you\'d like to backup (or "All Settings") and hit the "Download Export File" button.', 'fascat' ) . '</p>' .
		  '<p>' . __( 'To restore your settings from a backup, browse your computer for the file (under the "Import Settings" heading) and hit the "Upload File and Import" button. This will restore only the settings that have changed since the backup.', 'fascat' ) . '</p>' .
		  
		  '<p><strong>' . __( 'Please note that only valid backup files generated through the fascat Backup Manager should be imported.', 'fascat' ) . '</strong></p>' .

		  '<p><strong>' . __( 'Looking for assistance?', 'fascat' ) . '</strong></p>' .
		  '<p>' . sprintf( __( 'Please post your query on the %sfascat Support Desk%s where we will do our best to assist you further.', 'fascat' ), '<a href="http://support.fascat.com/" target="_blank">', '</a>' ) . '</p>';
	...
	public function admin_notices () {
		...
		echo '<div id="import-notice" class="updated"><p>' . sprintf( __( 'Please note that this backup manager backs up only your settings and not your content. To backup your content, please use the %sWordPress Export Tool%s.', 'fascat' ), '<a href="' . admin_url( 'export.php' ) . '">', '</a>' ) . '</p></div><!--/#import-notice .message-->' . "\n";
			
		if ( isset( $_GET['error'] ) && $_GET['error'] == 'true' ) {
			echo '<div id="message" class="error"><p>' . __( 'There was a problem importing your settings. Please Try again.', 'fascat' ) . '</p></div>';
		} else if ( isset( $_GET['error-export'] ) && $_GET['error-export'] == 'true' ) {  
			echo '<div id="message" class="error"><p>' . __( 'There was a problem exporting your settings. Please Try again.', 'fascat' ) . '</p></div>';
		} else if ( isset( $_GET['invalid'] ) && $_GET['invalid'] == 'true' ) {  
			echo '<div id="message" class="error"><p>' . __( 'The import file you\'ve provided is invalid. Please try again.', 'fascat' ) . '</p></div>';
		} else if ( isset( $_GET['imported'] ) && $_GET['imported'] == 'true' ) {  
			echo '<div id="message" class="updated"><p>' . sprintf( __( 'Settings successfully imported. | Return to %sTheme Options%s', 'fascat' ), '<a href="' . admin_url( 'admin.php?page=fascat' ) . '">', '</a>' ) . '</p></div>';
		} // End IF Statement
	...
	public function admin_screen_logic () {
		...
		if ( ! isset( $_POST['fascat-backup-export'] ) && isset( $_POST['fascat-backup-import'] ) && ( $_POST['fascat-backup-import'] == true ) ) {
			$this->import();
		}
		
		if ( ! isset( $_POST['fascat-backup-import'] ) && isset( $_POST['fascat-backup-export'] ) && ( $_POST['fascat-backup-export'] == true ) ) {
			$this->export();
		}
	...
	public function move_admin_menu () {
		...
		if ( ! array_key_exists( 'fascat', $submenu ) ) { return ; }
		
		$items_to_move = array();
		$first_item = array();
		$below_items = array();
		
		foreach ( $submenu['fascat'] as $k => $s ) {
			if ( in_array( $s[2], array( 'fascat-backup' ) ) ) {
				$items_to_move[] = $s;
				unset( $submenu['fascat'][$k] );
			}
			
			if ( in_array( $s[2], array( 'fascat_themes', 'fascat_timthumb_update' ) ) ) {
				$below_items[] = $s;
				unset( $submenu['fascat'][$k] );
			}
			
			if ( $k == 0 ) { $first_item[] = $s; unset( $submenu['fascat'][$k]); }
		}
		
		sort( $items_to_move );
		
		$remaining_items = $submenu['fascat'];
		
		// Grab the first item and unset it from the main array.
		$submenu['fascat'] = array_merge( $first_item, $remaining_items, $items_to_move, $below_items );
	} // End move_admin_menu()
	...
	public function import() {
		check_admin_referer( 'fascat-backup-import' ); // Security check.
		
		if ( ! isset( $_FILES['fascat-import-file'] ) ) { return; } // We can't import the settings without a settings file.
		
		// Extract file contents
		$upload = file_get_contents( $_FILES['fascat-import-file']['tmp_name'] );
		
		// Decode the JSON from the uploaded file
		$options = json_decode( $upload, true );
		
		// Check for errors
		if ( ! $options || $_FILES['fascat-import-file']['error'] ) {
			wp_redirect( admin_url( 'admin.php?page=' . $this->token . '&error=true' ) );
			exit;
		}
		
		// Make sure this is a valid backup file.
		if ( ! isset( $options['fascat-backup-validator'] ) ) {
			wp_redirect( admin_url( 'admin.php?page=' . $this->token . '&invalid=true' ) );
			exit;
		} else {
			unset( $options['fascat-backup-validator'] ); // Now that we've checked it, we don't need the field anymore.
		}
	...
	public function export() {
		global $wpdb;
		check_admin_referer( 'fascat-backup-export' ); // Security check.
		...
		$settings['fascat-backup-validator'] = date( 'Y-m-d h:i:s' );
