...
?>
<?php
// fascat Admin Interface

/*-----------------------------------------------------------------------------------

TABLE OF CONTENTS

- fascat Admin Interface - fascat_add_admin
- fascat Reset Function - woo_reset_options
- Framework options panel - fascat_options_page
- Framework Settings page - fascat_framework_settings_page
- woo_admin_head
- woo_load_only
- Ajax Save Action - woo_ajax_callback
- Generates The Options - fascat_machine
- fascat Uploader - fascat_uploader_function
- fascat Theme Version Checker - fascat_version_checker
- fascat Thumb Detection Notice - woo_thumb_admin_notice
- fascat Theme Update Admin Notice - woo_theme_update_notice
...
if ( ! function_exists( 'fascat_add_admin' ) ) {
	function fascat_add_admin() {
   	...
   	------------------------------------------------------------*/
  		...
  		------------------------------------------------------------*/
			...
			if ( $_action == 'reset' ) {
				...
				case 'fascat':
					...
					woo_reset_options( $options, 'fascat' );
					header( "Location: admin.php?page=fascat&reset=true" );
					die;

					break;

				case 'fascat_framework_settings':

					$options = get_option( 'woo_framework_template' );
					woo_reset_options( $options );
					header( "Location: admin.php?page=fascat_framework_settings&reset=true" );
					die;
				...
				case 'fascat_seo':
					...
					woo_reset_options( $options );
					header( "Location: admin.php?page=fascat_seo&reset=true" );
					die;
				...
				case 'fascat_sbm':
					...
					header( "Location: admin.php?page=fascat_sbm&reset=true" );
					die;
		...
		if( function_exists( 'add_object_page' ) ) {
			add_object_page ( 'Page Title', $themename, 'manage_options', 'fascat', 'fascat_options_page', $icon );
		} else {
			add_menu_page ( 'Page Title', $themename, 'manage_options', 'fascat_home', 'fascat_options_page', $icon );
		}
		$woopage = add_submenu_page( 'fascat', $themename, __( 'Theme Options', 'fascat' ), 'manage_options', 'fascat', 'fascat_options_page' ); // Default

		// Framework Settings Menu Item
		$wooframeworksettings = '';
		if( $super_user == $current_user_id || empty( $super_user ) ) {
			$wooframeworksettings = add_submenu_page( 'fascat', __( 'Framework Settings', 'fascat' ), __( 'Framework Settings', 'fascat' ), 'manage_options', 'fascat_framework_settings', 'fascat_framework_settings_page' );
		}

		// fascat Content Builder
		if ( function_exists( 'fascat_content_builder_menu' ) ) {
			fascat_content_builder_menu();
		}

		// Update Framework Menu Item
		if( $super_user == $current_user_id || empty( $super_user ) ) {
			$woothemepage = add_submenu_page( 'fascat', 'WooFramework Update', 'Update Framework', 'manage_options', 'fascat_framework_update', 'fascat_framework_update_page' );
		}
...
add_action( 'admin_menu', 'fascat_add_admin', 10 );
...
if ( ! function_exists( 'fascat_options_page' ) ) {
	function fascat_options_page() {
		global $pagenow;
...
?>
		...
		$return = apply_filters( 'woo_before_option_page', fascat_machine( $options ) );
?>
				...
				<div id="woo-nav">
					...
					<ul>
						<li class="top-level general current">
							<div class="arrow"><div></div></div><span class="icon"></span><a title="General Settings" href="#woo-option-error"><?php _e( 'Error', 'fascat' ); ?></a>
						</li>
					</ul>
				</div>
				<div id="content">
					<div class="group" id="woo-option-error" style="display: block; ">
						<div class="section section-info">
							<h3 class="heading"><?php _e( 'An Error Occured', 'fascat' ); ?></h3>
							<div class="option">
								<div class="controls">
									<p><?php _e( 'Something went wrong while trying to load your Theme Options panel.', 'fascat' ); ?></p>
									<p><?php echo sprintf( __( 'Please reload the page, if this error persists, please get in touch with us through our %1$s.', 'fascat' ), '<a href="' . esc_url( 'http://support.fascat.com' ) . '" target="_blank">' . __( 'Support Desk', 'fascat' ) . '</a>' ); ?></p>
								</div>
...
?>
			...
			jQuery( '#wooform' ).submit(function() {
					...
					var data = {
						<?php if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'fascat' ) { ?>
						type: 'options',
						<?php } ?>
						<?php if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'fascat_framework_settings' ) { ?>
						type: 'framework',
						<?php } ?>
						<?php if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'fascat_seo' ) { ?>
						type: 'seo',
						<?php } ?>
						<?php if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'fascat_tumblog' ) { ?>
						type: 'tumblog',
						<?php } ?>
...
function woo_admin_message_success () {
	echo '<div class="updated fade" style="display: block !important;"><p>' . __( 'Options Saved Successfully', 'fascat' ) . '</p></div><!--/.updated fade-->' . "\n";
} // End woo_admin_message_success()

function woo_admin_message_error () {
	echo '<div class="error fade" style="display: block !important;"><p>' . __( 'There was an error while saving your options. Please try again.', 'fascat' ) . '</p></div><!--/.error fade-->' . "\n";
} // End woo_admin_message_error()

function woo_admin_message_reset () {
	echo '<div class="updated fade" style="display: block !important;"><p>' . __( 'Options Reset Successfully', 'fascat' ) . '</p></div><!--/.updated fade-->' . "\n";
...
if ( ! function_exists( 'fascat_machine' ) ) {
	function fascat_machine( $options ) {
		$counter = 0;
		$menu = '';
		$output = '';
		...
		foreach ( $options as $k => $value ) {
			...
			case "upload":
				$output .= fascat_medialibrary_uploader( $value['id'], $value['std'], null ); // New AJAX Uploader using Media Library
				break;
				
			case "upload_min":
				$output .= fascat_medialibrary_uploader( $value['id'], $value['std'], 'min' ); // New AJAX Uploader using Media Library
				break;
			...
			case 'timestamp':
				...
				$output .= ' <span class="woo-timestamp-at">' . __( '@', 'fascat' ) . '</span> ';
			...
			if ( $value['type'] != "heading" && $value['type'] != "subheading" ) {
				...
				if ( !current_user_can( 'unfiltered_html' ) && isset( $value['id'] ) && in_array( $value['id'], woo_disabled_if_not_unfiltered_html_option_keys() ) )
					$explain_value .= '<br /><br /><b>' . esc_html( __( 'You are not able to update this option because you lack the <code>unfiltered_html</code> capability.', 'fascat' ) ) . '</b>';
		...
		if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] != 'fascat_content_builder' ) {
			$output .= '</div>';
		}
...
if ( ! function_exists( 'fascat_uploader_function' ) ) {
	function fascat_uploader_function( $id, $std, $mod ) {
		return fascat_medialibrary_uploader( $id, $std, $mod );
	} // End fascat_uploader_function()
}
...
function fascat_do_not_cache_feeds( &$feed ) { $feed->enable_cache( false ); } // End fascat_do_not_cache_feeds()
function fascat_http_request_args( $r ) { $r['timeout'] = 15; return $r; } // End fascat_http_request_args()
function fascat_http_api_curl( $handle ) { curl_setopt( $handle, CURLOPT_CONNECTTIMEOUT, 15 ); curl_setopt( $handle, CURLOPT_TIMEOUT, 15 ); } // End fascat_http_api_curl()

if ( ! function_exists( 'fascat_version_checker' ) ) {
	function fascat_version_checker ( $local_version ) {
		add_action( 'wp_feed_options', 'fascat_do_not_cache_feeds' );
		add_filter( 'http_request_args', 'fascat_http_request_args', 100, 1 );
		add_action( 'http_api_curl', 'fascat_http_api_curl', 100, 1 );
		...
		if ( $latest_version_via_rss == '' ) {
			$feed_url = 'http://www.fascat.com/?feed=updates&theme=' . $theme_name;
		...
		if( isset( $pieces_rss[0] ) && $pieces_rss[0] != 0 ) {
			...
			$statuses = array(
				'new_version' => __( 'New Version', 'fascat' ),
				'new_feature' => __( 'New Feature', 'fascat' ),
				'bugfix' => __( 'Bugfix', 'fascat' )
			);
			...
			if ( $version_sentinel == true ) {
				$update_message = '<div class="update_available status-' . $status . '">' . __( 'Theme update is available', 'fascat' ) . ' (v.' . $latest_version_via_rss['version'] . ') - <a href="http://www.fascat.com/products/">' . __( 'Get the new version', 'fascat' ) . '</a>.<p>' . sprintf( __( 'Update Type: %s', 'fascat' ), $statuses[$status] ) . '</p></div>';
			}
			else {
				$update_message = '';
			}
	...
	if( $super_user == $current_user_id || empty( $super_user ) ) {
		...
		if ( $thumb_php_test || $timthumb_php_test ) {
			echo '<div class="error">
    			   <p><strong>' . __( 'ATTENTION: A possible old version of the TimThumb script was detected in your theme folder. Please remove the following files from your theme as a security precaution', 'fascat' ) . ':</strong></p>';
...
global $pagenow;
if ( $pagenow == 'admin.php' && isset( $_GET['page'] ) && $_GET['page'] == 'fascat' ) {
...
if ( ! function_exists( 'woo_theme_update_notice' ) ) {
	function woo_theme_update_notice () {
		...
		$update_data = fascat_version_checker( $local_version );
		...
		if ( is_array( $update_data ) && $update_data['is_update'] == true ) {
			$html = '<div id="theme_update" class="updated fade"><p>' . sprintf( __( 'Theme update is available (v%s). %sDownload new version%s (%sSee Changelog%s)', 'fascat' ), $update_data['version'], '<a href="http://www.fascat.com/products/">', '</a>', '<a href="http://www.fascat.com/changelogs/' . $update_data['theme_name'] . '/changelog.txt" target="_blank" title="Changelog">', '</a>' ) . '</p></div>';
		}
...
/*-----------------------------------------------------------------------------------*/
/* fascat Framework Update Notice - woo_framework_update_notice */
/*-----------------------------------------------------------------------------------*/
/**
...
if ( ! function_exists( 'woo_framework_update_notice' ) ) {
	function woo_framework_update_notice () {
		...
		if ( is_array( $update_data ) && $update_data['is_update'] == true ) {
			$html = '<div id="wooframework_update" class="updated fade"><p>' . sprintf( __( 'WooFramework update is available (v%s). %sDownload new version%s (%sSee Changelog%s)', 'fascat' ), $update_data['version'], '<a href="' . admin_url( 'admin.php?page=fascat_framework_update' ) . '">', '</a>', '<a href="http://www.fascat.com/updates/functions-changelog.txt" target="_blank" title="Changelog">', '</a>' ) . '</p></div>';
		}
...
/*-----------------------------------------------------------------------------------*/
/* fascat Framework Critical Update Notice - woo_framework_critical_update_notice */
/*-----------------------------------------------------------------------------------*/
/**
...
if ( ! function_exists( 'woo_framework_critical_update_notice' ) ) {
	function woo_framework_critical_update_notice () {
		...
		if ( is_array( $update_data ) && $update_data['is_update'] == true && $update_data['is_critical'] == true ) {
			...
			$html = '<div id="wooframework_important_update" class="error fade"><p>' . sprintf( __( 'An important WooFramework update is available (v%s). %sDownload new version%s (%sSee Changelog%s)', 'fascat' ), $update_data['version'], '<a href="' . admin_url( 'admin.php?page=fascat_framework_update' ) . '">', '</a>' ) . '</p></div>';
		}
