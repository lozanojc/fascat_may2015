...
<?php
/*-----------------------------------------------------------------------------------*/
/* fascat Media Library-driven AJAX File Uploader Module */
/* 2010-11-05. */
/*
/* If we're on a fascat specific administration page, add Media Library Uploader
/* specific actions for CSS, JavaScript and several other functionalities.
/*-----------------------------------------------------------------------------------*/

if ( is_admin() ) {
	add_action( 'init', 'fascat_mlu_init' );
	add_action( 'admin_print_scripts', 'fascat_mlu_insidepopup' );
	add_filter( 'gettext', 'fascat_mlu_change_button_text', null, 2 );
	...
	if ( ( isset( $_REQUEST['page'] ) ) ) {
		...
		if ( ( $_page != '' && substr( $_page, 0, 3 ) == 'woo' ) || $is_posts_page ) {
			add_action( 'admin_print_styles', 'fascat_mlu_css', 0 );
			add_action( 'admin_print_scripts', 'fascat_mlu_js', 0 );
		}
}
...
/* fascat_mlu_init */
/*
/* Global init() function for the fascat Media Library-driven AJAX File Uploader.
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'fascat_mlu_init' ) ) {
	function fascat_mlu_init () {
		register_post_type( 'wooframework', array(
			'labels' => array(
				'name' => __( 'WooFramework Internal Container', 'fascat' ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'fascat_mlu_css' ) ) {
	function fascat_mlu_css () {
		$_html = '';
...
if ( ! function_exists( 'fascat_mlu_js' ) ) {
	function fascat_mlu_js () {
...
/*-----------------------------------------------------------------------------------*/
/* fascat_medialibrary_uploader */
/*
/* fascat Uploader Using the WordPress Media Library.
/*
/* Parameters:
...
/*
/* Dependencies:
/* - fascat_mlu_get_silentpost()
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'fascat_medialibrary_uploader' ) ) {
	function fascat_medialibrary_uploader ( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0 ) {
		$output = '';
		...
		if ( $_postid != 0 ) {
			...
			$int = fascat_mlu_get_silentpost( $id ); // Change for each field, using a "silent" post. If no post is present, one will be created.
		}
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'fascat' ) . '" rel="' . $int . '" />' . "\n";
		...
		return $output;
	} // End fascat_medialibrary_uploader()
}
...
/*
/* Example Usage:
/* fascat_mlu_get_silentpost ( 'woo_logo' );
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'fascat_mlu_get_silentpost' ) ) {
	function fascat_mlu_get_silentpost ( $_token ) {
		global $wpdb;
		...
		return $_id;
	} // End fascat_mlu_get_silentpost()
}

/*-----------------------------------------------------------------------------------*/
/* fascat_mlu_insidepopup */
/*
/* Trigger code inside the Media Library popup.
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'fascat_mlu_insidepopup' ) ) {
	function fascat_mlu_insidepopup () {
		if ( isset( $_REQUEST['is_fascat'] ) && $_REQUEST['is_fascat'] == 'yes' ) {
			add_action( 'admin_head', 'fascat_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'fascat_mlu_modify_tabs' );
		}
	} // End fascat_mlu_insidepopup()
}

if ( ! function_exists( 'fascat_mlu_js_popup' ) ) {
	function fascat_mlu_js_popup () {
		$_woo_title = 'file';
...
?>
	...
	jQuery(function($) {
		...
		// Preserve the "is_fascat" parameter on the "delete" confirmation button.
		$( '.savesend a.del-link' ).click ( function () {
			...
			continueHref = continueHref + '&is_fascat=yes';
...
<?php
	...
	} // End fascat_mlu_js_popup()
}

/*-----------------------------------------------------------------------------------*/
/* fascat_mlu_modify_tabs */
/*
/* Triggered inside the Media Library popup to modify the title of the "Gallery" tab.
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'fascat_mlu_modify_tabs' ) ) {
	function fascat_mlu_modify_tabs ( $tabs ) {
		if ( isset( $tabs['gallery'] ) ) { $tabs['gallery'] = str_replace( __( 'Gallery', 'fascat' ), __( 'Previously Uploaded', 'fascat' ), $tabs['gallery'] ); }
		return $tabs;
	} // End fascat_mlu_modify_tabs()
} // End IF Statement

/*-----------------------------------------------------------------------------------*/
/* fascat_mlu_change_button_text */
/*
/* Change the "Insert Into Post" button text where appropriate.
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'fascat_mlu_change_button_text' ) ) {
	function fascat_mlu_change_button_text( $translation, $original ) {
	    ...
	    if( $original == 'Insert into Post' ) {
	    	$translation = __( 'Use this Image', 'fascat' );
			if ( isset( $_REQUEST['title'] ) && $_REQUEST['title'] != '' ) { $translation = sprintf( __( 'Use as %s', 'fascat' ), esc_attr( $_REQUEST['title'] ) ); }
	    }
	
	    return $translation;
	} // End fascat_mlu_change_button_text()
}
?>