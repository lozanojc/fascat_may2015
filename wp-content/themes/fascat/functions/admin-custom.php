 ...
 * - fascat_metabox_create()

 * - fascat_metabox_create_fields()

 * - fascat_metabox_handle()

 * - fascat_metabox_add()

 * - fascat_metabox_fieldtypes()

 * - fascat_uploader_custom_fields()
...
function fascat_metabox_create( $post, $callback ) {
	...
	do_action( 'fascat_metabox_create', $post, $callback );
    ...
    if ( $callback['id'] == 'fascat-settings' ) {
	    ...
	    $output .= '<ul class="tabber hide-if-no-js">' . "\n";
	    	...
	    	if ( $display_general_fields ) {
                ...
                $output .= '<li class="wf-tab-general"><a href="#wf-tab-general">' . __( 'General Settings', 'fascat' ) . '</a></li>' . "\n";
    ...
    if ( $display_general_fields ) {
        ...
        $output .= fascat_metabox_create_fields( $woo_metaboxes, $callback, 'general' );
...
function fascat_metabox_create_fields ( $metaboxes, $callback, $token = 'general' ) {
    ...
    foreach ( $metaboxes as $k => $woo_metabox ) {
    	...
    	$woo_id = 'fascat_' . $woo_metabox['name'];
    	...
    	if ( $type_selector ) {
    		...
    		if( isset( $woo_metabox['type'] ) && ( in_array( $woo_metabox['type'], fascat_metabox_fieldtypes() ) ) ) {
...

        	    {
        	    	...
        	    	// Add support for the fascat Media Library-driven Uploader Module // 2010-11-09.
        	    	...
        	    	if ( function_exists( 'fascat_medialibrary_uploader' ) ) {
	    	            ...
	    	            $output .= "\t\t".'<td class="woo_metabox_fields">'. fascat_medialibrary_uploader( $woo_metabox['name'], $_value, 'postmeta', $woo_metabox['desc'], $post->ID );
	    	            ...
	    	            $output .= "\t\t".'<td class="woo_metabox_fields">'. fascat_uploader_custom_fields( $post->ID, $woo_name, $default, $woo_metabox['desc'] );
...
} // End fascat_metabox_create_fields()
...
function fascat_metabox_handle( $post_id ) {
    	...
    	if ( function_exists( 'get_post_types' ) ) {
			...
			foreach ( $custom_post_list as $type ) {
				...
				$settings = array(
									...
									'title' => sprintf( __( '%s Custom Settings', 'fascat' ), $theme_name ), 

									'callback' => 'fascat_metabox_create',
				...
				$settings = apply_filters( 'fascat_metabox_settings', $settings, $type, $settings['id'] );
    		...
    		add_meta_box( 'fascat-settings', sprintf( __( '%s Custom Settings', 'fascat' ), $theme_name ), 'fascat_metabox_create', 'post', 'normal' );

        	add_meta_box( 'fascat-settings', sprintf( __( '%s Custom Settings', 'fascat' ), $theme_name ), 'fascat_metabox_create', 'page', 'normal' );

    	}

    }

} // End fascat_metabox_add()
...
function fascat_metabox_fieldtypes() {

	return apply_filters( 'fascat_metabox_fieldtypes', array( 'text', 'calendar', 'time', 'time_masked', 'select', 'select2', 'radio', 'checkbox', 'textarea', 'images' ) );

} // End fascat_metabox_fieldtypes()
...
function fascat_uploader_custom_fields( $pID, $id, $std, $desc ) {
...
} // End fascat_uploader_custom_fields()
...
add_action( 'edit_post', 'fascat_metabox_handle', 10 );

add_action( 'admin_menu', 'fascat_metabox_add', 10 ); // Triggers fascat_metabox_create()
