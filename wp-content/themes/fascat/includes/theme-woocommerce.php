...
//add_action('woocommerce_before_main_content', 'woocommerce_fascat_before_content', 10);

//add_action('woocommerce_after_main_content', 'woocommerce_fascat_after_content', 20);



if (!function_exists('woocommerce_fascat_before_content')) {

	function woocommerce_fascat_before_content() {
        global $product;?>
<!-- #content Starts -->
...
<?php if(!FasCat::isCoaching($product->id)){?>
            <ul id="submenu">
...
if (!function_exists('woocommerce_fascat_after_content')) {

function woocommerce_fascat_after_content() {
global $product;
if(!FasCat::isCoaching($product->id)){
    comments_template();
    //var_dump(FasCat::getAttachments($product));
    if(false !== ($attachments = FasCat::getAttachments($product))){$xx = 0;
        foreach($attachments as $img){?>
<?php $xx++;if($xx > 1){break;}
}?>
<div class="clear"></div>
<?php }
...
function woo_wc_search_form( $form ) {
    ...
    <label class="screen-reader-text" for="s">' . __( 'Search Products:' , 'fascat' ) . '</label>

    <input type="search" results=5 autosave="'. esc_url(home_url( '/' )) .'" class="input-text" placeholder="'. esc_attr__( 'Search Products', 'fascat' ) .'" value="' . get_search_query() . '" name="s" id="s" />

    <input type="submit" class="button" id="searchsubmit" value="'. esc_attr__( 'Search', 'fascat' ) .'" />
...
add_filter( 'woocommerce_output_related_products_args', 'fascat_related_products' );

function fascat_related_products() {
...
add_action('fascat_homepage_content', 'fascat_product_categories', 10);

add_action('fascat_homepage_content', 'fascat_featured_products', 10);

add_action('fascat_homepage_content', 'fascat_recent_products', 30);



function fascat_product_categories() {

	global $woo_options;

	$params = apply_filters( 'fascat_homepage_product_categories_params', 'number=""' );

	if (class_exists('woocommerce') && $woo_options[ 'woo_homepage_product_categories' ] == "true" ) {

		echo '<h1>'.__('Product Categories', 'fascat').'</h1>';
...
function fascat_recent_products() {

	global $woo_options;

	if (class_exists('woocommerce') && $woo_options[ 'woo_homepage_products' ] == "true" ) {

		echo '<h1 class="center_aligned"> <span>'.__('Newest Products', 'fascat').'</span> </h1>';
...
function woocommerce_cart_link() {
	...
	<a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'fascat'); ?>" class="cart-parent">

		<span>

	<?php

	echo '<span class="contents">' . sprintf(_n('%d', '%d', $woocommerce->cart->get_cart_contents_count(), 'fascat'), $woocommerce->cart->get_cart_contents_count()) . '</span>';
