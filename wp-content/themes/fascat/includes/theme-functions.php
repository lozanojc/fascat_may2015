...
if ( function_exists( 'wp_nav_menu') ) {
	...
	register_nav_menus( array( 'primary-menu' => __( 'Primary Menu', 'fascat' ) ) );

	register_nav_menus( array( 'top-menu' => __( 'Top Menu', 'fascat' ) ) );
	...
	function woo_pagenav() {
		...
		?>
            ...
            <nav class="nav-entries fix">
                ...
                <?php next_posts_link( '<span class="nav-prev fl">'. __( '<span class="meta-nav">&larr;</span> Older posts', 'fascat' ) . '</span>' ); ?>

                <?php previous_posts_link( '<span class="nav-next fr">'. __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'fascat' ) . '</span>' ); ?>
...
<aside class="post-meta">
	...
	<ul>
		...
		<?php if ( isset( $woo_options['woo_post_content'] ) && $woo_options['woo_post_content'] == 'excerpt' ) { ?>
			...
			<li class="comments"><?php comments_popup_link( __( 'Leave a comment', 'fascat' ), __( '1 Comment', 'fascat' ), __( '% Comments', 'fascat' ) ); ?></li>

		<?php } ?>

		<?php edit_post_link( __( 'Edit', 'fascat' ), '<li class="edit">', '</li>' ); ?>
...
if (!function_exists( 'woo_subscribe_connect')) {
	...
	function woo_subscribe_connect($widget = 'false', $title = '', $form = '', $social = '', $contact_template = 'false') {
		...
		$settings = array(
						...
						'connect_title' => __('Subscribe' , 'fascat'), 

						'connect_related' => 'true', 

						'connect_content' => __( 'Subscribe to our e-mail newsletter to receive updates.', 'fascat' ),
...
?>
	...
	<aside id="connect" class="fix">
		...
		<h3><?php if ( $title ) echo apply_filters( 'widget_title', $title ); else _e('Subscribe','fascat'); ?></h3>
		...
		<div <?php if ( $related_posts != '' ) echo 'class="col-left"'; ?>>
			...
			<form class="newsletter-form<?php if ( $related_posts == '' ) echo ' fl'; ?>" action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open( 'http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $settings[ 'connect_newsletter_id' ]; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520' );return true">
				...
				<input class="email" type="text" name="email" value="<?php esc_attr_e( 'E-mail', 'fascat' ); ?>" onfocus="if (this.value == '<?php _e( 'E-mail', 'fascat' ); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e( 'E-mail', 'fascat' ); ?>';}" />
				...
				<input class="submit" type="submit" name="submit" value="<?php _e( 'Submit', 'fascat' ); ?>" />
			...
			<div id="mc_embed_signup">
				...
				<form class="newsletter-form<?php if ( $related_posts == '' ) echo ' fl'; ?>" action="<?php echo $settings['connect_mailchimp_list_url']; ?>" method="post" target="popupwindow" onsubmit="window.open('<?php echo $settings['connect_mailchimp_list_url']; ?>', 'popupwindow', 'scrollbars=yes,width=650,height=520');return true">
					...
					<input type="text" name="EMAIL" class="required email" value="<?php _e('E-mail','fascat'); ?>"  id="mce-EMAIL" onfocus="if (this.value == '<?php _e('E-mail','fascat'); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e('E-mail','fascat'); ?>';}">

					<input type="submit" value="<?php _e('Submit', 'fascat'); ?>" name="subscribe" id="mc-embedded-subscribe" class="btn submit button">
		...
		<div class="related-posts col-right">
			...
			<h4><?php _e( 'Related Posts:', 'fascat' ); ?></h4>
...
/*-----------------------------------------------------------------------------------*/
	...
	if ( ! function_exists( 'woo_comment_form_fields' ) ) {
		...
		function woo_comment_form_fields ( $fields ) {
			...
			$required_text = ' <span class="required">(' . __( 'Required', 'fascat' ) . ')</span>';
...
/*-----------------------------------------------------------------------------------*/
	...
	if ( ! function_exists( 'woo_comment_form_settings' ) ) {
		...
		function woo_comment_form_settings ( $settings ) {
			...
			$settings['label_submit'] = __( 'Submit Comment', 'fascat' );

			$settings['cancel_reply_link'] = __( 'Click here to cancel reply.', 'fascat' );
...
add_action( 'fascat_add_javascript', 'woo_load_prettyphoto', 10 );

add_action( 'fascat_add_css', 'woo_load_prettyphoto', 10 );
